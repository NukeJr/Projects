...
class BusinessContact(Contact):
    ...
    def __str__(self):
        return f"Name: {self.name}, number: {self.phone_number}, email: {self.email}, company: {self.company}, Job title: {self.job_title}" 
...
class B_ContactBook:
    def __init__(self):
        self.Bcontacts = [BusinessContact("Gearld", phone_number="801-123-1234", email="bob@gmail.com", company="Lukes Nukes", job_title= "Mr.Bun")]

    def badd_contact(self, Bcontact: BusinessContact):
        self.Bcontacts.append(Bcontact)

    def bdelete_contact(self, name):
        for Bcontact in self.bsearch_contact:
            if Bcontact.name == name:
                self.Bcontacts.remove(Bcontact)
                return True
        return False

    def bsearch_contact(self, name):
        for Bcontact in self.Bcontacts:
            if Bcontact.name == name:
                return Bcontact
        return None

    def bdisplay_contacts(self):
        if len(self.Bcontacts) == 0:
        ...
        if choice == "6":
            ...
            while True: 
                ...
                if validate_email(email):
                    ...
                    print("Invalid email. Please try again.")
            pass
            bcontact = (name, phone_number, email, )
            B_ContactBook.badd_contact(bcontact)
            clear_screen()
            if choice == "6":
                print(f"Successfully added {bcontact} as a new Buiness contact!!")
            ...
            clear_screen()
            print("Here are all of your Buisness contacts.")
            B_contacts.bdisplay_contacts()                      
        else:
            clear_screen()
